...
void converter2(int nevts = 0, TString prefix = "signal", TString out = "TREEOUT")
{
 	// *** some variables
	int i=0;
	...
	fRun->AddFriend("signal_sim.root");
...
//  ====================================================== Branches end   ====================================================== //
	...
	while (theAnalysis->GetEvent() && i++<nevts)
	{
		...
		theAnalysis->FillList( muminus, "PionTightMinus");
		theAnalysis->FillList( kminus, "KaonAllMinus");
		theAnalysis->FillList( pminus, "ProtonAllMinus");
		
		
		ALL += all.GetLength();
		PIwo += piminus.GetLength();
		PIwi += muminus.GetLength();

		    
		for (int j=0; j<piminus.GetLength(); ++j)
		{
			//double theta = ((pminus[j]->P3().Theta())*180)/PI;
			//cout<<"Theta: "<<theta<<" degrees"<<endl;
			if (theAnalysis->McTruthMatch(pminus[j]))
			{
				PndPidCandidate *myCand = (PndPidCandidate*) pminus[j]->GetRecoCandidate();                
	...
	t1.Write();
	cout<<"all length is: "<< ALL<<endl;
	cout<<"piwo length is: "<< PIwo<<endl;
	cout<<"allwi length is: "<< PIwi<<endl;
} // macro end
